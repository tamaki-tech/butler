...
export async function createEventReminder(
  ...
  if (!parsedStart) {
    return {
      ok: false,
      message:
        "日時の形式が正しくありません。例: `2026-12-25 19:00` または `12/25 19:00`",
    };
  }
  const scheduledStartTime = parsedStart.date;
  if (parsedStart.inferredYear) {
    autoCompletionNotes.push(
      `開始日時の年を ${scheduledStartTime.getFullYear()} 年として解釈しました`,
    );
  }
  ...
  if (input.end) {
    const parsedEnd = parseDateTime(
      input.end,
      scheduledStartTime.getFullYear(),
    );
    if (!parsedEnd) {
      return {
        ok: false,
        message:
          "終了日時の形式が正しくありません。例: `2026-12-25 21:00` または `12/25 21:00`",
      };
    }
    if (parsedEnd.inferredYear) {
      autoCompletionNotes.push(
        `終了日時の年を ${parsedEnd.date.getFullYear()} 年として解釈しました`,
      );
    }
  ...
  if (participants.length > MAX_PARTICIPANTS) {
    return {
      ok: false,
      message: `リマインド対象は最大${MAX_PARTICIPANTS}人までです。`,
    };
  }
  if (!input.participants?.trim()) {
    autoCompletionNotes.push(
      "リマインド対象が未指定のため、対象なしで登録しました",
    );
  }
  ...
  const fullDescription = updateDescriptionWithMeta(
    input.description ?? "",
    botMeta,
  );
  ...
  try {
    ...
    const autoCompletionMessage =
      autoCompletionNotes.length > 0
        ? [
            "",
            "**自動補完した内容**",
